...
IMPORTANT: Some paths may only exist BECAUSE of a bug (e.g., a path where duplicates are dropped, or a path where unbalanced parens return True). Write assertions for what a CORRECT implementation should return for that input, NOT what the buggy path produces.
