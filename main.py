...
def main():
    ...
    for repo in REPOS:
        ...
        # Clone and extract target functions
        ...
        # Generate tests, run them, attempt fixes
    ...
    try:
        ...
        for bug in bugs:
            ...
            try:
                ...
                # Find the functions touched by the bug patch
        ...
        # Save per-run results and print summary
...
def run_for_functions(functions, output_dir, repo_dir):
    ...
    # Generate whitebox and blackbox tests for each function
    print("\nGenerating tests...")
    ...
    # Run all generated tests and collect outcomes
    print("\nRunning tests...")
    test_outcomes = {}
    ...
    # Attempt one-shot fixes for any failing functions
...
def _one_shot_fix(fn, fn_key, test_outcomes, repo_dir, output_dir, idx):
    """Ask the LLM to diagnose failures, apply a fix, and revise test oracles if needed."""
    ...
    # If tests still fail after the fix, the oracles may have been written for the buggy behavior —
    # ask the LLM to revise them to match the corrected function.
    if remaining:
...
def _run_fn_tests(fn_key, output_dir, repo_dir):
    """Run all test files for a single function and return outcomes dict."""
    outcomes = {}
...
def _is_valid_fix(code, fn_name):
    """Return True if `code` looks like a valid function fix (not a test file, parses cleanly)."""
...
def _load_bugs(bugsinpy_dir, projects, limit_per_project):
    """Walk the BugsInPy metadata directory and return a list of bug dicts."""
    bugs = []
...
def _functions_from_patch(patch_text, repo_path):
    """Return the functions in repo_path that are touched by patch_text."""
    files = {}
    current_file = None
